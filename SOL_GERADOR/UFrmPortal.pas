...
type
  TFrmPortal = class(TFrmModeloSis)
    CbxLoja: TComboBox;
    lblLoja: TLabel;
...
procedure TFrmPortal.GerarProducao;
begin
  inherited;
...
procedure TFrmPortal.GerarProduto;
var
   codigoBarra : String;
   count, loja4 : Integer;
   TotalCount : Integer;
...
procedure TFrmPortal.GerarSecao;
begin
  inherited;
...
procedure TFrmPortal.GerarSubGrupo;
begin
  inherited;
...
procedure TFrmPortal.GerarVenda;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.BtnAmarrarCestClick(Sender: TObject);
begin
  inherited;
    inherited;
...
procedure TFrmPortal.btnGeraCestClick(Sender: TObject);
begin
  inherited;
  FlgGeraCest := True;
  BtnGerar.Click;
...
procedure TFrmPortal.BtnGerarClick(Sender: TObject);
begin
...
procedure TFrmPortal.GerarCest;
var
   count : integer;
begin
  inherited;
...
procedure TFrmPortal.GerarCliente;
begin
  ...
  begin
    ...
    SQL.Clear;
     ...
     SQL.Add('       COALESCE(REPLACE(REPLACE(REPLACE(CLIENTES.CNPJ_CPF, ''.'', ''''), ''-'', ''''), ''/'', ''''), '''') AS NUM_CGC,   ');
     SQL.Add('       COALESCE(REPLACE(REPLACE(REPLACE(CLIENTES.IE, ''.'', ''''), ''-'', ''''), ''/'', ''''), ''ISENTO'') AS NUM_INSC_EST,   ');
     ...
     SQL.Add('       CLIENTES.RENDA AS VAL_RENDA,   ');
     ...
     SQL.Add('       COALESCE(CLIENTES.DTCAD, '''') AS DTA_CADASTRO,   ');
     SQL.Add('       CLIENTES.NUMERO AS NUM_ENDERECO,   ');
     SQL.Add('       COALESCE(CLIENTES.RG, '''') AS NUM_RG,   ');
     SQL.Add('          ');
     ...
     SQL.Add('       COALESCE(CLIENTES.DTALTERA, '''') AS DTA_ALTERACAO,   ');
     SQL.Add('       COALESCE(CLIENTES.OBS1, '''') AS DES_OBSERVACAO,   ');
     ...
     SQL.Add('       COALESCE(CLIENTES.DTANIVER, '''') AS DTA_NASCIMENTO,   ');
     SQL.Add('       COALESCE(CLIENTES.NOMEPAI, '''') AS DES_PAI,   ');
     SQL.Add('       COALESCE(CLIENTES.NOMEMAE, '''') AS DES_MAE,   ');
     SQL.Add('       COALESCE(CLIENTES.NOMECONJUGE, '''') AS DES_CONJUGE,   ');
     SQL.Add('       COALESCE(CLIENTES.CPF_CONJUGE, '''') AS NUM_CPF_CONJUGE,   ');
     SQL.Add('       0 AS VAL_DEB_CONV,   ');
     SQL.Add('          ');
     SQL.Add('       CASE CLIENTES.ATIVO    ');
...
procedure TFrmPortal.GerarCodigoBarras;
var
...
procedure TFrmPortal.GerarComposicao;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarCondPagCli;
begin
  inherited;
...
procedure TFrmPortal.GerarCondPagForn;
begin
  inherited;
  ...
  begin
    ...
    SQL.Clear;
     ...
     SQL.Add('       COALESCE(REPLACE(REPLACE(REPLACE(FORNECEDORES.CNPJ_CPF, ''.'', ''''), ''-'', ''''), ''/'', ''''), '''') AS NUM_CGC   ');
     SQL.Add('   FROM   ');
...
procedure TFrmPortal.GerarDecomposicao;
begin
  inherited;
...
procedure TFrmPortal.GerarDivisaoForn;
begin
  inherited;
    with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarFinanceiro(Tipo, Situacao: Integer);
begin
  inherited;
  if Tipo = 1 then
...
procedure TFrmPortal.GerarFinanceiroPagar(Aberto: String);
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarFinanceiroReceber(Aberto: String);
var
   codParceiro : Integer;
   numDocto : String;
   count : integer;
...
procedure TFrmPortal.GerarFinanceiroReceberCartao;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarFornecedor;
var
...
begin
  ...
  begin
    ...
    SQL.Clear;
     ...
     SQL.Add('       COALESCE(FORNECEDORES.FANTASIA, '''') AS DES_FANTASIA,   ');
     SQL.Add('       COALESCE(REPLACE(REPLACE(REPLACE(FORNECEDORES.CNPJ_CPF, ''.'', ''''), ''-'', ''''), ''/'', ''''), '''') AS NUM_CGC,   ');
     SQL.Add('       COALESCE(REPLACE(REPLACE(REPLACE(FORNECEDORES.IE, ''.'', ''''), ''-'', ''''), ''/'', ''''), ''ISENTO'') AS NUM_INSC_EST,   ');
     ...
     SQL.Add('       FORNECEDORES.ESTADO AS DES_SIGLA,   ');
     SQL.Add('       FORNECEDORES.CEP AS NUM_CEP,   ');
     SQL.Add('       COALESCE(FORNECEDORES.TELEFONE, '''') AS NUM_FONE,   ');
     SQL.Add('       COALESCE(FORNECEDORES.FAX, '''') AS NUM_FAX,   ');
     SQL.Add('       COALESCE(FORNECEDORES.CONTATO, '''') AS DES_CONTATO,   ');
     ...
     SQL.Add('       ''N'' AS MICRO_EMPRESA,   ');
     SQL.Add('       FORNECEDORES.CODFORNEC AS COD_FORNECEDOR_ANT,   ');
     SQL.Add('       COALESCE(FORNECEDORES.NUMERO, ''S/N'') AS NUM_ENDERECO,   ');
     SQL.Add('       COALESCE(FORNECEDORES.OBS, '''') AS DES_OBSERVACAO,   ');
     SQL.Add('       COALESCE(FORNECEDORES.EMAIL, '''') AS DES_EMAIL,   ');
     SQL.Add('       '''' AS DES_WEB_SITE,   ');
     SQL.Add('       ''N'' AS FABRICANTE,   ');
     SQL.Add('       COALESCE(FORNECEDORES.PRODRURAL, ''N'') AS FLG_PRODUTOR_RURAL,   ');
     SQL.Add('       0 AS TIPO_FRETE,   ');
     SQL.Add('       ''N'' AS FLG_SIMPLES,   ');
     SQL.Add('       ''N'' AS FLG_SUBSTITUTO_TRIB,   ');
     SQL.Add('       0 AS COD_CONTACCFORN,   ');
     SQL.Add('          ');
     SQL.Add('       CASE FORNECEDORES.ATIVO   ');
     SQL.Add('           WHEN ''S'' THEN ''N''   ');
     SQL.Add('           ELSE ''S''   ');
     SQL.Add('       END AS INATIVO,   ');
     SQL.Add('      ');
     SQL.Add('       0 AS COD_CLASSIF,   ');
     SQL.Add('       COALESCE(FORNECEDORES.DTCAD, '''') AS DTA_CADASTRO,   ');
     SQL.Add('       0 AS VAL_CREDITO,   ');
     SQL.Add('       0 AS VAL_DEBITO,   ');
     SQL.Add('       1 AS PED_MIN_VAL,   ');
     SQL.Add('       '''' AS DES_EMAIL_VEND,   ');
     SQL.Add('       COALESCE(FORNECEDORES.SENHACOTACAO, '''') AS SENHA_COTACAO,   ');
...
procedure TFrmPortal.GerarGrupo;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarInfoNutricionais;
begin
  inherited;
...
procedure TFrmPortal.GerarNCMUF;
var
 count : Integer;
begin
  inherited;
...
procedure TFrmPortal.GerarNFClientes;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarNFFornec;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarNFitensClientes;
begin
  inherited;
  with QryPrincipal2 do
  begin
...
procedure TFrmPortal.GerarNFitensFornec;
var
...
procedure TFrmPortal.GerarProdForn;
begin
  inherited;
...
procedure TFrmPortal.GerarProdLoja;
var
  count, count1 : Integer;
begin
  inherited;
...
procedure TFrmPortal.GerarProdSimilar;
begin
  inherited;
  with QryPrincipal2 do
  begin
