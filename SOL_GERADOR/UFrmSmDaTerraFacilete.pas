...
//  end;
  ...
  begin
    ...
    SQL.Clear;
     ...
     SQL.Add('       END AS NUM_INSC_EST,   ');
     SQL.Add('       CASE      ');
     SQL.Add('           WHEN ((C.CLI_ENDERECO = '''') OR (C.CLI_ENDERECO IS NULL)) THEN ''Á DEFINIR''      ');
     SQL.Add('           ELSE C.CLI_ENDERECO      ');
     SQL.Add('       END AS DES_ENDERECO,      ');
     SQL.Add('       CASE      ');
     SQL.Add('           WHEN ((C.CLI_BAIRRO = '''') OR (C.CLI_BAIRRO IS NULL)) THEN ''Á DEFINIR''      ');
     SQL.Add('           ELSE C.CLI_BAIRRO      ');
     SQL.Add('       END AS DES_BAIRRO,      ');
     SQL.Add('       CASE      ');
     SQL.Add('           WHEN ((C.CLI_CIDADE = '''') OR (C.CLI_CIDADE IS NULL)) THEN ''SAO PEDRO''      ');
     SQL.Add('           ELSE REPLACE (C.CLI_CIDADE, ''Ã'', ''A'')      ');
...
//  end;
  ...
  begin
    ...
    SQL.Clear;
       ...
       SQL.Add('         END AS NUM_INSC_EST,   ');
       SQL.Add('         CASE   ');
       SQL.Add('           WHEN ((F.FOR_ENDERECO = '''') OR (F.FOR_ENDERECO IS NULL)) THEN ''Á DEFINIR''   ');
       SQL.Add('           ELSE F.FOR_ENDERECO   ');
       SQL.Add('         END AS DES_ENDERECO,   ');
       SQL.Add('         CASE   ');
       SQL.Add('           WHEN ((F.FOR_BAIRRO = '''') OR (F.FOR_BAIRRO IS NULL)) THEN ''Á DEFINIR''   ');
...
begin
  ...
  begin
    ...
    SQL.Clear;
     ...
     SQL.Add('       G.GRU_CODIGO AS COD_SECAO,   ');
     SQL.Add('       ''999'' AS COD_GRUPO,   ');
     SQL.Add('       ''Á DEFINIR'' AS DES_GRUPO,   ');
     SQL.Add('       0 AS VAL_META   ');
     SQL.Add('      ');
     SQL.Add('   FROM GRUPO G   ');
...
begin
  ...
  begin
    ...
    SQL.Clear;
     ...
     SQL.Add('       CASE   ');
     SQL.Add('           WHEN ((CF.CFIS_DESCRICAO = '''') OR (CF.CFIS_DESCRICAO IS NULL)) THEN ''Á DEFINIR''   ');
